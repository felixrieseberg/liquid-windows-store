var WebBlender = function ($) {
    ...
    */
    ...
    var loader = '<div class="spinnerSpacer">' +
        '<div class="loader">' +
        '<div class="circle">' +
        '</div>' +
        '<div class="circle">' +
    ...
    */
    ...
    /**
      Gets client-specific js download.
      @private
    */
    var getClientJs = function (clientType, environment, flight) {
        return $.get(getUrl(environment, '/getClientJS', flight), { ClientType: clientType }, function (data) {
            parentElement.append('<script type="text/javascript" src="' + data + '"></script>');
        });
    };
    ...
    */
    var openBlend = function (postBody, parentElementId, environment, flight, clientType, hash, iframeOptions) {
        if (!htmlFetched) {
            ...
            var useiframe = iframeOptions && iframeOptions.useiframe;

            if (useiframe) {
                getClientJs(clientType, environment, flight, parentElementId).then(function() {
                    var start = new Date().getTime();
                    // TODO: figure out what to do now with client JS... does it go in the iframe?
                    ...
                    window.postMessage(JSON.stringify({ message: 'error', data: 'blenderSDK: Unable to load client JS' }), '*');
                });
            } else {
                ...
                getClientJs(clientType, environment, flight, parentElementId).then(function() {
                    ...
                    $.post(url, postBody, function(data) {
                        parentElement.html(parentElement.html() + data);
                        resetHtmlFetched(true);
